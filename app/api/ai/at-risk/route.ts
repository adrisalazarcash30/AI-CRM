...
import { rankAtRisk, type BriefingDeal } from "@/lib/anthropic";
...
export async function GET() {
  ...
  const snapshot: BriefingDeal[] = (deals ?? []).map((d) => {
  ...
  const items = await rankAtRisk(snapshot);
  return NextResponse.json({ items, snapshot });
}
